...
/**
 * Orchestrateur dédié au job `jsonConversionJob`.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class JsonConversionOrchestrator {

    private final GenericOrchestrator genericOrchestrator;

    public void runOnce(String type, String documentId) throws Exception {
        genericOrchestrator.runOnce("jsonConversionJob", type, documentId);
    }

    public void runContinuous(String type, String documentId, long intervalMillis, boolean stopOnFailure) {
        genericOrchestrator.runContinuous("jsonConversionJob", type, documentId, intervalMillis, stopOnFailure);
    }
}
