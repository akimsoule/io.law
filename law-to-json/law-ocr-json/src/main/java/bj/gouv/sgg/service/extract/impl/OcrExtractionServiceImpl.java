...
public class OcrExtractionServiceImpl implements OcrExtractionService {
    ...
    public double calculateConfidence(String text, List<Article> articles, String documentId) {
        ...
        if (log.isDebugEnabled()) {
            String debugMsg = String.format(java.util.Locale.ROOT,
                    "Confidence calculation: articles=%.3f, sequence=%.3f, text=%.3f, dict=%.3f (unrecRate=%.2f%%, penalty=%.2f%%), legal=%.3f → total=%.3f",
                    articleScore, sequenceScore, textLengthScore, dictScore, unrecRate * 100, unrecPenalty * 100, legalScore, confidence);
            log.debug(debugMsg);

            if (!unrecognizedWords.isEmpty()) {
                log.debug("📝 Found {} unrecognized words (rate: {}%)",
                        unrecognizedWords.size(), String.format(java.util.Locale.ROOT, "%.2f", unrecRate * 100));
            }
        }
