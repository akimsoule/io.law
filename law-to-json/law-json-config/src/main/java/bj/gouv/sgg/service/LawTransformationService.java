...
public class LawTransformationService {
    ...
    public LawTransformationService(
    ...
    public JsonResult transform(LawDocument document, Path pdfPath) throws IAException {
        ...
        double ocrQualityThreshold = getOcrThreshold();
        double jsonQualityThreshold = getJsonThreshold();
    ...
    private String readOcrText(LawDocument document) throws IAException {
        try {
            Path ocrPath = config.getOcrDir()
                ...
                .resolve(document.getDocumentId() + ".txt");
            
            if (!Files.exists(ocrPath)) {
                throw new IAException("Fichier OCR introuvable: " + ocrPath);
            }
    ...
    private int extractArticleCount(JsonResult result) {
        try {
            JsonObject root = com.google.gson.JsonParser.parseString(result.getJson()).getAsJsonObject();
    ...
    /**
     * Récupère le seuil de qualité OCR depuis la configuration.
     */
    private double getOcrThreshold() {
        return config.getOcrQualityThreshold();
    }
