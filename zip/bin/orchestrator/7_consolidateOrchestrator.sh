#!/bin/bash
# Usage: ./orchestrator/7_consolidateOrchestrator.sh [type]
...
cd "$PROJECT_ROOT"
java -jar "$JAR" --spring.config.location="file:$CONFIG" --orchestrator=consolidateOrchestrator --mode=once --type="$TYPE" 2>&1 | tee "$LOG_FILE"
