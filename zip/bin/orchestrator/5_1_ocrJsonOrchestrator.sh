#!/bin/bash
# Usage: ./orchestrator/5_1_ocrJsonOrchestrator.sh [type]
...
cd "$PROJECT_ROOT"
java -jar "$JAR" --spring.config.location="file:$CONFIG" --orchestrator=ocrJsonOrchestrator --mode=once --type="$TYPE" 2>&1 | tee "$LOG_FILE"
