#!/bin/bash
# Usage: ./orchestrator/3_downloadOrchestrator.sh [type]
...
cd "$PROJECT_ROOT"
java -jar "$JAR" --spring.config.location="file:$CONFIG" --orchestrator=downloadOrchestrator --mode=once --type="$TYPE" 2>&1 | tee "$LOG_FILE"
