...
TYPE="${1:-loi}"
DOCUMENT_ID="${2:-loi-2017-25}"

mkdir -p "$PROJECT_ROOT/logs"
cd "$PROJECT_ROOT"
java -jar "$JAR" --spring.config.location="file:$CONFIG" --spring.profiles.active=batch --job=ocrJsonJob --type="$TYPE" --documentId="$DOCUMENT_ID" 2>&1 | tee "$LOG_FILE"