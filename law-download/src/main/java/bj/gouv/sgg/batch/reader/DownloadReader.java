 ...
 * 
 * @StepScope permet injection des paramètres du job (type, documentId,
 *            maxItems)
 */
@Slf4j
@Component
...
public class DownloadReader implements ItemReader<LawDocumentEntity> {
    ...
    @Value("#{jobParameters['maxItems'] ?: 2000}")
    ...
    private synchronized void initialize() {
        ...
        if (documentId != null && !documentId.isEmpty() && !"ALL".equals(documentId)) {
            ...
            repository.findByDocumentId(documentId).ifPresentOrElse(
                    doc -> {
                        ...
                        if (validator.isNotDownloaded(doc)) {
                            documents.add(doc);
        ...
        List<LawDocumentEntity> found = repository.findByTypeAndStatusIn(type, List.of(
                ProcessingStatus.FETCHED,
                ProcessingStatus.FAILED_CORRUPTED,
                ProcessingStatus.FAILED_DOWNLOAD));
